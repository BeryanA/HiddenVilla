    ...
    public class HotelRoomRepository : IHotelRoomRepository
    {
        ...
        public Task<IEnumerable<HotelRoomDTO>> GetAllHotelRooms()
        {
            try
            {
                ...
                return Task.FromResult(hotelRoomDTOs);
            }
            catch (Exception ex)
            {
                return null;
        ...
        public async Task<HotelRoomDTO> GetHotelRoom(int roomId)
        {
            try
            {
                HotelRoomDTO hotelRoom =
                    _mapper.Map<HotelRoom, HotelRoomDTO>(await _db.HotelRooms.FirstOrDefaultAsync(x => x.Id == roomId));
                return hotelRoom;
            }
            catch (Exception ex)
            {
        ...
        public async Task<HotelRoomDTO> IsRoomUnique(string name)
        {
            ...
            {
                HotelRoomDTO hotelRoom =
                    _mapper.Map<HotelRoom, HotelRoomDTO>(await _db.HotelRooms.FirstOrDefaultAsync(x => x.Name == name));
                return hotelRoom;
            }
            catch (Exception ex)
            {
